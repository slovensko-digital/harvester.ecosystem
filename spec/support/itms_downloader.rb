RSpec.shared_context "itms_downloader", :shared_context => :metadata do
  ...
  before do
    ...
    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/dodavatelia/'))
      .and_return(double(body: itms_file_fixture('dodavatel_item.json')))
    ...
    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/verejneObstaravania/'))
      .and_return(double(body: itms_file_fixture('verejne_obstaravanie_item.json')))

    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/zmluvaVerejneObstaravanie/'))
      .and_return(double(body: itms_file_fixture('zmluva_verejne_obstaravanie_item.json')))

    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/vyzvy/planovane'))
      .and_return(double(body: itms_file_fixture('vyzva_planovana_item.json')))

    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/vyzvy/vyhlasene'))
      .and_return(double(body: itms_file_fixture('vyzva_vyhlasena_item.json')))

    allow(downloader)
      .to receive(:get)
      .with(include('https://opendata.itms2014.sk/v2/polozkaRozpoctu/'))
  ...
  def id_specific_accounting_document_fixture(url)
    ...
    fixture = JSON.parse(itms_file_fixture('uctovny_doklad_item.json'))
    fixture['polozkyDokladu'].map { |pd| pd['id'] = "#{itms_id}#{pd['id']}".to_i }
    double(body: JSON.generate(fixture))
  end
end
