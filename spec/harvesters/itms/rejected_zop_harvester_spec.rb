...
RSpec.describe Itms::RejectedZopHarvester, type: :service do
  ...
  context '#run' do
    it 'loads and imports all rejected zop' do
      ...
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/zop/zamietnute?minId=0&limit=100')
        .and_return(double('response', response_code: 200, body: zop_content)).at_least(:once)
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/zop/zamietnute?minId=1404159811923837200&limit=100')
