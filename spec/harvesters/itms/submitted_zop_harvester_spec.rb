...
RSpec.describe Itms::SubmittedZopHarvester, type: :service do
  ...
  context '#run' do
    it 'loads and imports all submitted zop' do
      ...
      }.to change(Itms::SubmittedZop, :count).by(1)

      zop = Itms::SubmittedZop.first
      ...
      expect(zop.submitted_zop_pre_financing).to have_attributes(
        itms_identifier: 1404159811923837200
      )
      expect(zop.submitted_zop_submitted_for).to have_attributes(
        dic: 'Eligendi id sit.',
      ...
      expect(zop.submitted_zop_recipient).to have_attributes(
        dic: 'Eligendi id sit.',
        ico: 'Velit omnis provident aperiam occaecati corrupti quibusdam.',
        itms_identifier: 1264743098737356000,
        ine_identifikacne_cislo: 'Rerum qui voluptatem harum aut eaque.'
      )
      expect(zop.submitted_zop_project).to have_attributes(
      ...
      expect {
        described_class.run(min_id: 0, downloader: downloader)
      }.to change(Itms::SubmittedZop, :count).by(0)
    end
  end
end
