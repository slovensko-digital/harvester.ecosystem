...
RSpec.describe Itms::DeclaredAppealsHarvester, type: :service do
  ...
  context '#run' do
    it 'loads and imports all planned appeals' do
      ...
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/vyzvy/vyhlasene?minId=0&limit=100')
        .and_return(double('response', response_code: 200, body: appeals_content)).at_least(:once)
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/vyzvy/vyhlasene?minId=5608104377301085000&limit=100')
