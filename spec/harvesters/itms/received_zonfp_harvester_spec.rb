...
RSpec.describe Itms::ReceivedZonfpHarvester, type: :service do
  ...
  context '#run' do
    it 'loads and imports all received zonfp' do
      ...
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/zonfp/prijate?minId=0&limit=100')
        .and_return(double('response', response_code: 200, body: zonfp_content)).at_least(:once)
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/zonfp/prijate?minId=6737247880451281000&limit=100')
