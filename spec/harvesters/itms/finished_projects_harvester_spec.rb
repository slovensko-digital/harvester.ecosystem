...
RSpec.describe Itms::FinishedProjectsHarvester, type: :service do
  ...
  context '#run' do
    it 'loads and imports all finished projects' do
      ...
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/projekty/ukoncene?minId=0&limit=100')
        .and_return(double('response', response_code: 200, body: file_content)).at_least(:once)
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/projekty/ukoncene?minId=6058429222335320000&limit=100')
