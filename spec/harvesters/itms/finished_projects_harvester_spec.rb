...
RSpec.describe Itms::FinishedProjectsHarvester, type: :service do

  context '#run' do
    it 'loads and imports all finished projects' do
      file_content = File.read(fixture_filepath('fixtures/files/projekty.json'))
      ...
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/v1/projekty/ukoncene?minId=0&limit=100')
        .and_return(double('response', response_code: 200, body: file_content)).at_least(:once)
      expect(downloader).to receive(:get).with('https://opendata.itms2014.sk/v1/projekty/ukoncene?minId=6058429222335320000&limit=100')
      ...
      }.to change(Itms::FinishedProject, :count).by(1)

      finished_project = Itms::FinishedProject.first
      expect(finished_project).to have_attributes(
      ...
      expect(finished_project.finished_project_activities.first).to have_attributes(
      ...
      expect(finished_project.finished_project_activities.first.finished_project_activity_unit).to have_attributes(
      ...
      expect(finished_project.finished_project_financing_forms.first).to have_attributes(
        itms_identifier: 6619364344456191000,
        kod_zdroj: 'Ut ipsum.',
        nazov: 'Dolores quo recusandae blanditiis quaerat dignissimos minima.'
      )
      expect(finished_project.finished_project_financing_forms.first.finished_project_financing_form_goal).to have_attributes(
        itms_identifier: 2139854021380057000
      )
      expect(finished_project.finished_project_economic_activities.first).to have_attributes(
        itms_identifier: 6619364344456191000,
        kod_zdroj: 'Ut ipsum.',
        nazov: 'Dolores quo recusandae blanditiis quaerat dignissimos minima.'
      )
      expect(finished_project.finished_project_economic_activities.first.finished_project_economic_activity_goal).to have_attributes(
        itms_identifier: 2139854021380057001
      )
      expect(finished_project.finished_project_intensities.first).to have_attributes(
        itms_identifier: 5158007136973555000,
        nazov: 'Voluptatem ipsa voluptate.'
      )
      expect(finished_project.finished_project_intensities.first.finished_project_intensity_unit).to have_attributes(
        itms_identifier: 3604616086874745000,
        ico: 'Atque error ad perferendis soluta autem.',
        ine_identifikacne_cislo: 'Omnis provident aperiam occaecati corrupti.'
      )
      expect(finished_project.finished_project_intensities.first.finished_project_intensity_sources.first).to have_attributes(
        typ: 'zdrojEu',
      ...
      expect(finished_project.finished_project_measurable_indicators.first).to have_attributes(
      ...
      expect(finished_project.finished_project_realization_places.first.finished_project_realization_place_units.first).to have_attributes(
        typ: 'nuts3',
        itms_identifier: 9214145182608512000,
        nazov: 'Aut officiis est eaque quos.'
      )
      expect(finished_project.finished_project_monitoring_dates.first).to have_attributes(
      ...
      expect(finished_project.finished_project_intervention_areas.first).to have_attributes(
        itms_identifier: 6619364344456191000,
        kod_zdroj: 'Ut ipsum.',
        nazov: 'Dolores quo recusandae blanditiis quaerat dignissimos minima.'
      )
      expect(finished_project.finished_project_intervention_areas.first.finished_project_intervention_area_goal).to have_attributes(
        itms_identifier: 2139854021380057002
      )
      expect(finished_project.finished_project_organizational_components.first).to have_attributes(
        adresa: 'Temporibus repudiandae nobis et vel in.',
        itms_identifier: 4750311352290475000,
        nazov: 'Unde itaque sunt eius voluptas aut.'
      )
      expect(finished_project.finished_project_partners.first).to have_attributes(
        ico: 'Atque error ad perferendis soluta autem.',
        itms_identifier: 3604616086874745000,
        ine_identifikacne_cislo: 'Omnis provident aperiam occaecati corrupti.'
      )
      expect(finished_project.finished_project_recipient).to have_attributes(
        itms_identifier: 3604616086874745000,
        ico: '123123123',
        ine_identifikacne_cislo: 'ine_123123123_cislo'
      )
      expect(finished_project.finished_project_territory_types.first).to have_attributes(
        itms_identifier: 6619364344456191000,
        kod_zdroj: 'Ut ipsum.',
        nazov: 'Dolores quo recusandae blanditiis quaerat dignissimos minima.'
      )
      expect(finished_project.finished_project_territory_types.first.finished_project_territory_type_goal).to have_attributes(
        itms_identifier: 2139854021380057004
      )
      expect(finished_project.finished_project_territorial_mechanisms.first).to have_attributes(
        itms_identifier: 6619364344456191000,
        kod_zdroj: 'Ut ipsum.',
        nazov: 'Dolores quo recusandae blanditiis quaerat dignissimos minima.'
      )
      expect(finished_project.finished_project_territorial_mechanisms.first.finished_project_territorial_mechanism_goal).to have_attributes(
        itms_identifier: 2139854021380057005
      )
      expect(finished_project.finished_project_appeal).to have_attributes(
      ...
      expect {
        described_class.run(min_id: 0, downloader: downloader)
      }.to change(Itms::FinishedProject, :count).by(0)
    end
  end
end
