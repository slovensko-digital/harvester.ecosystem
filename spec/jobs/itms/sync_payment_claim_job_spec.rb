...
RSpec.describe Itms::SyncPaymentClaimJob, type: :job do
  ...
  def sync_rejected_payment_claim
    expect(downloader)
      .to receive(:get_json_from_href)
      .with('/v2/zop/predlozene/123')
      .and_return(itms_json_fixture('zop_predlozena_item.json'))
  ...
  describe '#perform' do
    ...
    context 'when state changes from paid to rejected' do
      it 'resets attributes specific to paid state' do
        sync_paid_payment_claim
        sync_rejected_payment_claim
    ...
    context 'when state changes from rejected to paid' do
