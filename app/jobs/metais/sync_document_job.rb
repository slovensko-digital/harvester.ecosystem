...
class Metais::SyncDocumentJob < ApplicationJob
  ...
  def parse_document(document_version, json, meta)
    ...
    document_version.typ_dokumentu = get_document_attribute(json, 'Profil_Dokument_Projekt_typ_dokumentu')
    ...
    document_version.mime_type = meta['mimeType']
    document_version.content_length = meta['contentLength']
    document_version.status = meta['status']
